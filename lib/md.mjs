...
function buildUploadStats(data, today) {
  const raw = (data.mdUploads || data.mdProducts || [])
    .map((p) => ({
      date: dateKey(p.date),
      uploader: String(p.uploader || "").trim(),
    }))
    .filter((p) => p.date && p.date <= today);
  const rawNames = [...new Set(raw.map((p) => p.uploader).filter(Boolean))];
  const normalizeUploader = (name) => {
    if (!name) return "";
    const base = name.replace(/[＊*]+$/u, "").trim();
    const longerMatches = rawNames.filter(
      (candidate) =>
        candidate !== name &&
        !/[＊*]+$/u.test(candidate) &&
    ...
    return longerMatches.length === 1 ? longerMatches[0] : name;
  };
  const source = raw.map((p) => ({
    ...p,
    uploader: normalizeUploader(p.uploader),
  }));
