export const SOURCING_STATE_COPY = Object.freeze({
  exploration_signal: {
    label: "탐색 신호",
    description: "여러 상품에서 주문 반응이 확인된 상태입니다. 반복성과 최근 흐름, 집중도를 함께 확인하세요.",
  },
...
function readinessView(readiness) {
  ...
  return {
    requirements: (readiness.requirements || []).map((item) => ({
      key: item.key,
      label: item.label,
      status: item.status,
      statusLabel: READINESS_STATUS_COPY[item.status] || "미확인",
      ready: Boolean(item.ready),
      detail: item.detail,
    })),
    coverageStart: readiness.coverage?.start || null,
    coverageThrough: readiness.coverage?.through || null,
    recentComparisonStart: readiness.recentComparisonStart || null,
    registrationDates: readiness.registrationDates || null,
    seasonEnds: readiness.seasonEnds || null,
    upstreamIssues: readiness.upstreamIssues || null,
    validatedRequirementCount: readiness.validatedRequirementCount ?? 0,
    totalRequirementCount: readiness.totalRequirementCount ?? 0,
    activationReady: Boolean(readiness.strongActivationReady && readiness.weakActivationReady),
    note: readiness.note || "",
  };
}
...
function recentSentence(group) {
  ...
  if (flags.has("mixed_recent") && d30 && d14) {
    return `최근 30일 주문 ${d30.Q.previous ?? "?"} → ${d30.Q.current ?? "?"}개 · 최근 14일 판단 반영 수량 ${d14.E.previous ?? "?"} → ${d14.E.current ?? "?"}개로 지표가 엇갈립니다.`;
  }
...
function narrative(group, displayState = group.primaryState) {
  ...
  if (displayState === "evidence_strong") {
    return `${L ?? "?"}개 연결 상품 중 ${A ?? "?"}개에서 주문 반응이 확인됐고, 서로 다른 날짜에 반복된 상품은 ${D ?? "확인 필요"}개입니다. 데이터 검증 조건과 강한 근거 게이트를 모두 충족한 관측 상태입니다.`;
  }
  if (displayState === "concentration_dependent") {
    ...
    const residual = top?.remainingRepeat === null
      ? ""
      : ` 해당 상품을 제외하면 다른 날짜에 판단 반영 반응이 반복된 상품은 ${top.remainingRepeat}개입니다.`;
    return `${L ?? "?"}개 연결 상품 중 ${A ?? "?"}개에서 주문 반응이 확인됐습니다. ${concentration}${residual}`;
  }
  if (displayState === "exploration_signal") {
...
function keyFacts(group, displayState = group.primaryState) {
  const all = group.periods?.all || {};
  ...
  if (displayState === "concentration_dependent" && top) {
...
export function buildSourcingView(diagnostics = {}, validationResult = null) {
  const groups = diagnostics.groups || [];
  const validationMap = new Map((validationResult?.groups || []).map((item) => [item.groupId, item]));
  ...
  const cards = clusters.map(({ representative: group, aliases }) => {
    ...
    const displayState = validation?.resolvedState || group.primaryState;
    const state = SOURCING_STATE_COPY[displayState] || SOURCING_STATE_COPY.insufficient_data;
    ...
    return {
      ...
      filterBuckets: [...new Set([group, ...aliases].map((item) => filterBucket(item)))],
      primaryState: displayState,
      observedState: group.primaryState,
      promoted: Boolean(validation?.promoted),
      ...
      narrative: narrative(group, displayState),
      recentSentence: recentSentence(group),
      keyFacts: keyFacts(group, displayState),
  ...
  const readiness = readinessView(diagnostics.sourceMeta?.readiness);
  ...
  return {
    summary: {
      ...
      coverageVerified: diagnostics.sourceMeta?.coverageVerified ?? groups.every((group) => group.dataFitness?.coverageVerified),
      exposureAvailable: diagnostics.sourceMeta?.exposureAvailable ?? groups.every((group) => group.dataFitness?.exposureKnown),
      testDurationAvailable: diagnostics.sourceMeta?.testDurationAvailable ?? groups.every((group) => group.dataFitness?.testDurationKnown),
    ...
    states: [
      { key: "all", label: "전체 상태" },
      { key: "evidence_strong", label: "강한 근거" },
