...
function assess(g, snapshot, policy) {
  ...
  const gates = {
    ...
    residualReplication: !dependent && !ambiguous && usable,
  ...
  const groups = makeGroups(ps, combinations).map(g => {
    ...
    const out = { id: g.id, level: g.level, label: g.label, definition: structuredClone(g.definition), memberKeys: [...memberSet].sort(),
      ...
      dataFitness: { coverageVerified: snapshot.collectionCompleteness === 'verified', exposureKnown: known('exposure'), testDurationKnown: known('testDuration'), availabilityKnown: known('availability'),
        taxonomyPartial: g.members.some(p => categoryFields.some(f => p.taxonomy[f] === '미확인') || Object.values(p.taxonomy.attributes).some(a => ['conflict', 'unscoped'].includes(a.status))) || scopedIssues.some(i => /TAXONOMY|CATEGORY/.test(i.code)),
        optionUnallocated: g.members.some(p => Object.values(p.taxonomy.attributes).some(a => a.status === 'option')),
        blockingIssues: scopedIssues.some(i => ['LINK_CONFLICT', 'TAXONOMY_VERSION_MISMATCH', 'PRODUCT_ORDER_CONFLICT', 'UPSTREAM_ISSUE'].includes(i.code)) } };
