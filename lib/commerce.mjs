...
export function parseOrders(rows, warnings = [], mode = "line") {
  ...
  rows.slice(1).forEach((row, i) => {
    const o = parseOrder(row, ix, i + 2, mode, warnings);
    if (!o) return;
...
export function parseClaims(rows, orders, warnings = []) {
  ...
  rows.slice(1).forEach((row, i) => {
    ...
    if (!original) {
      warnings.push(`취소 반품 ${i + 2}행: 원주문 미매칭, 차감 보류`);
      return;
    }
    const status = get(row, ix, "주문상태"),
    ...
    const qty = valid
      ...
      : (!quantityValue || quantityValue === "-") && original.qty === 1
        ...
        : null;
    if (qty === null)
      warnings.push(`취소 반품 ${i + 2}행: 부분 취소 수량 확인 필요`);
    const c = {
      ...original,
      type,
    ...
    if (
      ...
      warnings.push(`취소 반품 ${i + 2}행: 중복 클레임 충돌, 차감 보류`);
  ...
  for (const o of orders)
    if (
      ...
      if (!complete)
        warnings.push(`주문 ${o.row}행: 취소·반품 수량이 없어 차감 보류`);
...
export function parseInventory(rows, warnings = []) {
...
export function parseMd(rows, warnings = []) {
  ...
  for (const { ri, ci } of starts) {
    ...
    for (let r = Math.max(0, ri - 3); r < endRow; r++)
      for (let c = ci; c < nextCol; c++) {
        ...
        if (p[field] && p[field] !== v)
          warnings.push(`MD ${ri + 2}행: ${field} 중복 라벨 확인 필요`);
        p[field] = ["date", "registeredAt", "seasonEnd"].includes(field)
          ? dateKey(v)
          : field === "productNo"
            ? identifier(v)
            : numberOrNull(v);
...
export function buildCommerce(ov, cv, iv, mv, options = {}) {
  ...
  const orders = parseOrders(ov, warnings, mode);
  ...
  if (
    ...
    for (const o of parseOrders(cv, warnings, mode))
      if (
  ...
  const claims = parseClaims(cv, orders, warnings),
    inventory = parseInventory(iv, warnings),
    mdProducts = parseMd(mv, warnings);
