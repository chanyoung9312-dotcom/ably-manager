...
// Older claim rows omit the delivery memo cell. Repair only this known suffix
// signature; never slide identifiers, quantities, or money by guessing offsets.
export function normalizeClaimRows(rows) {
  const ix = columns(rows), si = ix("주문상태"), ti = ix("배송타입");
  if (si < 1 || ti !== si + 1 || ix("배송메모") !== si - 1) return rows;
  return rows.map((row, i) => {
    if (!i || !/^(일반배송|빠른배송|샥출발)$/.test(compact(row[si])) ||
        orderState(row[si - 1]) === "unknown" || text(row[ti])) return row;
    return [...row.slice(0, si - 1), "", ...row.slice(si - 1)];
  });
}
export function parseClaims(
  ...
  rows.slice(1).forEach((row, i) => {
    ...
    const isDate = (v) => {
      const s = text(v);
      return !!dateKey(s) || (/^\d{4}$/.test(s) && !!original.date &&
        !!dateKey(`${original.date.slice(0, 4)}-${s.slice(0, 2)}-${s.slice(2)}`));
    };
    ...
    // Request dates are shared by cancellations and returns in the live sheet.
    // A reason describes intent, never proof that processing is complete.
    const explicitCancel = state === "cancelled" || /취소|^C\d\d$/.test(compact(status));
    const explicitReturn = state === "returned" || /반품|^R\d\d$/.test(compact(status));
    const originalCancelled = original.state === "cancelled";
    const originalReturned = original.state === "returned";
    const claimedProduct = identifier(get(row, ix, "상품번호"));
    const productConflict = !!claimedProduct && !!original.productNo && claimedProduct !== original.productNo;
    const stateConflict =
      (originalCancelled && explicitReturn) ||
      (originalReturned && explicitCancel);
    const kind = explicitCancel || originalCancelled ? "cancel"
      : explicitReturn || originalReturned || returnDone ? "return"
      : /취소|배송\s*지연|품절|답변\s*없음|미답변/.test(reason) ? "cancel"
      : "unknown";
    const isReturn = kind === "return";
    const invalidStatus = !!status && state === "unknown";
    if (stateConflict || invalidStatus || productConflict)
      issue(warnings, issues,
        `취소 반품 ${i + 2}행: ${productConflict ? "원주문과 상품번호 불일치" : stateConflict ? "원주문과 취소·반품 상태 충돌" : "주문상태 형식 확인 필요"}`,
        productConflict ? [original.productNo, claimedProduct] : [original.productNo]);
    ...
    const type = isReturn
      ...
      : procurement && !["-", "취소", "미발주", "발주전"].includes(procurement)
        ...
        : procurement
          ? "before"
          : "unknown";
    const complete = !stateConflict && !productConflict && (isReturn
      ? state === "returned" || originalReturned || returnDone
      : state === "cancelled" || originalCancelled);
    ...
    if (
      old &&
      ["kind", "type", "complete", "qty", "claimStatus"].some((k) => old[k] !== c[k])
    ) {
      grouped.set(id, {
        ...c,
        complete: false,
  ...
  for (const o of orders)
    if (
      ...
      grouped.set(o.productOrder, {
        ...o,
        kind: o.state === "returned" ? "return" : "cancel",
...
export function buildCommerce(ov, cv, iv, mv, options = {}) {
  ...
  if (
    ...
    // Validate only moved originals. Copied claim rows are not new orders and
    // must not produce order errors that get suppressed later by row number.
    const claimIx = columns(cv);
    const moved = [cv[0], ...cv.slice(1).map((r) =>
      sourceIds.has(identifier(get(r, claimIx, "상품주문번호"))) ? [] : r)];
    for (const o of parseOrders(moved, warnings, mode, issues))
      if (
  ...
  for (const o of orders) {
    ...
    o.pendingClaim = c ? !c.complete || c.qty === null : o.state === "claim";
    o.eligibleQty =
