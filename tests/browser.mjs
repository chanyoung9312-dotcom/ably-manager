...
let browser;
const evidence =
  process.env.BROWSER_EVIDENCE_DIR || "/tmp/oars-browser-evidence";
...
try {
  ...
  const d = buildCommerce(
    [
      h,
      ...["2026-09-17", "2026-09-15", "2026-09-13"].map((date, i) => [
        date,
        `po${i}`,
        "order1",
        "p1",
  ...
  if (process.env.AGENT_BROWSER_CLI) {
    const cli = (...args) =>
      execFileSync(process.execPath, [process.env.AGENT_BROWSER_CLI, ...args], {
        env: {
          ...process.env,
    ...
    try {
      ...
      assert.match(
        cli(
          "eval",
          "document.querySelector('[data-nextjs-dialog]')?'ERROR':'OK'",
        ),
        /OK/,
      );
    } finally {
  ...
  if (process.env.BROWSER_FONT_DIR) {
    ...
    fontCss =
      (await readFile(root + "/400.css", "utf8")).replaceAll(
        "./files/",
        "/__test_fonts/",
      ) +
      '\nbody,button,input,select,textarea{font-family:"Noto Sans KR",sans-serif!important}';
    await page.route("**/__test_fonts/*", async (r) =>
      r.fulfill({
        body: await readFile(
          root +
            "/files/" +
  ...
  await page.route("**/api/oars-analysis", (r) =>
  ...
  await page.route("**/api/google-orders", (r) =>
    r.fulfill({ json: { rows: [ship] } }),
  );
  await page.route("**/api/cafe24/orders", (r) =>
    r.fulfill({
      json: {
        rows: [
          {
  ...
  let prepareBody;
  await page.route("**/api/cafe24/prepare", async (r) => {
    prepareBody = r.request().postDataJSON();
  ...
  await applyFont();
  await page.getByText("12. 🎯 오늘 해야 할 일 TOP 5").waitFor();
  await page.screenshot({ path: evidence + "/md-mobile.png", fullPage: true });
  await page.getByRole("button", { name: "문자 도우미", exact: true }).click();
  await page
    .getByRole("button", { name: "카페24 주문 불러오기", exact: true })
    .click();
  await page
    .getByRole("button", { name: "배송준비", exact: true })
    .first()
    .click();
  assert.deepEqual(prepareBody.orderItemCodes, ["item1"]);
  assert.equal(
    await page.getByRole("button", { name: "배송준비", exact: true }).count(),
    1,
  );
  await page.getByRole("button", { name: "송장 매칭", exact: true }).click();
  await page
    .getByRole("button", { name: "우체국 배송 주문 불러오기", exact: true })
    .click();
  await page.locator("textarea").fill("1234567890123 테스트고객");
  await page.getByRole("button", { name: "등기번호 자동 매칭" }).click();
  assert.equal(
    await page
      .getByRole("button", { name: "주문시트 송장번호 자동 기입" })
      .count(),
    0,
  );
  await page.locator("textarea").fill("1234567890123 테스트고객 01000000000");
  await page.getByRole("button", { name: "등기번호 자동 매칭" }).click();
  await page.route("**/api/tracking", (r) =>
    r.fulfill({ status: 409, json: { error: "주문 정보가 변경되었습니다." } }),
  );
  await page
    .getByRole("button", { name: "주문시트 송장번호 자동 기입" })
    .click();
  await page
    .getByText("주문 정보가 변경되었습니다.", { exact: false })
    .waitFor();
  await page.unroute("**/api/tracking");
  let trackingBody;
  await page.route("**/api/tracking", (r) => {
    trackingBody = r.request().postDataJSON();
    return r.fulfill({ json: { ok: true, count: 1, unchanged: 0 } });
  });
  await page
    .getByRole("button", { name: "주문시트 송장번호 자동 기입" })
    .click();
  await page.getByText("송장 1건 저장·재확인 완료", { exact: false }).waitFor();
  assert.equal(trackingBody.updates[0].productOrderNo, "po0");
  assert.ok(trackingBody.updates[0].snapshot);
  const wb = XLSX.utils.book_new(),
    ws = XLSX.utils.aoa_to_sheet([
      ["상품주문번호", "택배사", "택배사 코드", "송장번호", "보존필드"],
      ["po0", "기존택배", 1, "", "원본"],
      ["other", "기존택배", 1, "9999999999999", "다른 주문"],
    ]);
  ws.E2 = { t: "n", f: "1+1", v: 2 };
  XLSX.utils.book_append_sheet(wb, ws, "배송 중 관리");
  ...
  await page
    .getByRole("button", { name: "에이블리 원본 엑셀 송장 변경 다운로드" })
    .click();
  const download = await downloadPromise;
  const exported = XLSX.read(await readFile(await download.path()), {
    type: "buffer",
  }).Sheets["배송 중 관리"];
  assert.equal(exported.D2.v, "1234567890123");
  assert.equal(exported.E2.f, "1+1");
  assert.equal(exported.D3.v, "9999999999999");
  await page.getByRole("button", { name: "MD 총분석", exact: true }).click();
  await page.unroute("**/api/oars-analysis");
  await page.route("**/api/oars-analysis", (r) =>
    r.fulfill({ status: 502, json: { error: "모의 API 실패" } }),
  );
  await page.getByRole("button", { name: "새로고침", exact: true }).click();
  await page.getByRole("alert").filter({ hasText: "모의 API 실패" }).waitFor();
  assert.deepEqual(errors, []);
  console.log(
    "PASS: 8 routes × 2 viewports, auth/CSRF, single-item prepare, ambiguous matching, failed/successful tracking, Excel preservation, API error UX; no page errors.",
  );
} finally {
