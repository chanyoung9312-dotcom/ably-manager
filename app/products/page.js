...
export default function Products() {
  ...
  const stockedRows = useMemo(
    () => allRows.filter((row) => row.stock !== null && row.stock > 0),
    [allRows],
  );

  const rows = useMemo(() => {
    const filtered = stockedRows.filter((row) =>
  ...
  }, [stockedRows, query, sort]);

  const totalStock = stockedRows.reduce((sum, row) => sum + (row.stock || 0), 0);

  return (
    <main className="inventory-page">
      <header className="inventory-head">
        <div>
          <small>현재 보유 중인 재고만 표시</small>
          <h1>재고 현황</h1>
          <p>재고 현황 시트에서 수량이 1개 이상인 상품과 옵션만 보여줍니다.</p>
        </div>
      ...
      <section className="inventory-stats" aria-label="재고 요약">
        <div><span>재고 보유 상품</span><b>{stockedRows.length}개</b></div>
      ...
      <section className="inventory-list">
        <div className="inventory-list-head"><h2>보유 재고</h2></div>
        {rows.length ? rows.map((row) => {
          const options = (row.options || []).filter(
            (option) => !option.invalid && option.qty !== null && option.qty > 0,
          );
          return (
            <article className="inventory-row" key={row.key}>
              ...
              <div className="inventory-metrics">
                <div className="stock-main"><span>현재 재고</span><b>{row.stock}개</b></div>
                <div><span>재고 옵션</span><b>{options.length}개</b></div>
              ...
              <details>
                ...
                <div className="option-grid">
                  {options.map((option, i) => (
                    <div className="option-card" key={`${option.color}-${option.size}-${i}`}>
        ...
        }) : <p className="empty">현재 보유 중인 재고가 없습니다.</p>}
      </section>

      <style jsx>{`
        .inventory-page{max-width:1080px;margin:auto;padding:26px 18px 70px;color:inherit}.inventory-head{display:flex;justify-content:space-between;align-items:flex-start;gap:18px;flex-wrap:wrap}.inventory-head small,.inventory-head p{color:#aeb5b8}.inventory-head h1{font-size:34px;margin:5px 0}.inventory-head p{margin:0}.inventory-head button{padding:11px 16px;border-radius:12px}.inventory-stats{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin:24px 0 14px;max-width:560px}.inventory-stats div{border:1px solid #343a3d;border-radius:15px;padding:16px;background:#191d1f;display:flex;flex-direction:column;gap:4px}.inventory-stats span{color:#aeb5b8;font-size:13px}.inventory-stats b{font-size:24px}.inventory-toolbar{display:grid;grid-template-columns:1fr 230px;gap:10px;margin-bottom:14px}.inventory-toolbar label{display:flex;flex-direction:column;gap:6px}.inventory-toolbar label>span{font-size:12px;color:#aeb5b8}.inventory-toolbar input,.inventory-toolbar select{width:100%;min-height:46px;padding:0 13px;border-radius:11px;border:1px solid #3a4043;background:#191d1f;color:inherit}.inventory-list{border:1px solid #343a3d;border-radius:18px;background:#191d1f;padding:18px}.inventory-list-head{padding:2px 2px 14px;border-bottom:1px solid #303638}.inventory-list-head h2{margin:0;font-size:21px}.inventory-row{padding:20px 2px;border-bottom:1px solid #303638}.inventory-row:last-child{border-bottom:0}.inventory-title small{color:#929b9f}.inventory-title h3{margin:5px 0 0;font-size:18px;line-height:1.45}.inventory-metrics{display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin:15px 0}.inventory-metrics>div{background:#22282a;border-radius:12px;padding:12px;display:flex;flex-direction:column;gap:3px}.inventory-metrics .stock-main{background:#252d29}.inventory-metrics span{color:#aeb5b8;font-size:12px}.inventory-metrics b{font-size:19px}.inventory-row details{margin-top:12px}.inventory-row summary{cursor:pointer;font-weight:800}.option-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px;margin-top:12px}.option-card{padding:11px 12px;border-radius:11px;background:#22282a;display:flex;justify-content:space-between;gap:10px}.option-card span{font-size:13px;color:#c2c8ca}.empty{color:#aeb5b8;padding:16px 2px;margin:0}@media(max-width:760px){.inventory-toolbar{grid-template-columns:1fr}.inventory-metrics{grid-template-columns:1fr 1fr}.inventory-head h1{font-size:28px}.inventory-list{padding:15px}}@media(max-width:440px){.inventory-stats{grid-template-columns:1fr}.inventory-metrics{grid-template-columns:1fr}.option-grid{grid-template-columns:1fr}}
      `}</style>
    </main>
  );
}
