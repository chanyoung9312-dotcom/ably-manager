...
function ImageCard({ item, onPatch, onSaveCrop }) {
  ...
  return (
    <>
      ...
      <style jsx>{`
        .image-card{min-width:0;overflow:hidden;border:1px solid #354044;border-radius:16px;background:#171c1e}
        .image-card.crop{border-color:#8a7546}
        .image-card.bad{border-color:#694549}
        ...
        .crop-control>div:first-child{display:flex;justify-content:space-between;gap:8px;align-items:flex-start}
        .crop-control span{font-size:10px;color:#919ca0;text-align:right}
        .crop-control input{width:100%;margin:10px 0}
        .presets{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:4px}
        .presets button{padding:6px 2px;min-height:32px;font-size:10px}
        .save{width:100%;margin-top:8px;background:#e9eef0;color:#111719;font-weight:900}
        @media(max-width:560px){
          .card-body{padding:9px}
          .state-buttons button{font-size:10px}
          .crop-control>div:first-child{display:block}
          .crop-control span{display:block;text-align:left;margin-top:4px}
        }
      `}</style>
    </>
  );
  ...
  return (
    <main className="registration-helper">
      ...
      <section className="guide">
        ...
        <p><b>바로 판매 가능한 등록:</b> 정리한 메인·상세 이미지를 자동 업로드하고, 카페24에는 진열함·판매함 상태로 생성한 뒤 상품번호와 상태를 다시 확인합니다.</p>
      </section>
