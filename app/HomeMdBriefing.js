...
function Card({p}){
  const label=p.decision==="사입 검토"?"🔥 사입 검토":p.decision==="추가 사입 중단"?"⛔ 추가사입 중단":"👀 관찰";
  const seven=p.windows?.find(w=>w.days===7)?.current;
  return <article className="op-card">
    <div className="op-card-head"><b>{p.name||"상품명 미확인"}</b><span>{label}</span></div>
    <p className="numbers">최근 7일 <strong>{seven?.net??0}개 판매</strong> · 판매일 {p.activeDays??0}일 · 재고 {p.stock===null?"확인 필요":`${p.stock}개`}</p>
    <p className="decision">{p.buy||p.reason||p.observe}</p>
    {p.observe&&<small>다음 확인: {p.observe}</small>}
    <details><summary>상세 근거 보기</summary>
      <p>7일/14일/30일 순판매 {p.windows?.map(w=>`${w.current.net}개`).join(" / ")}</p>
      <p>최근 30일 취소·반품 {p.cancels}개 · 주문 집중도 {Math.round((p.concentration||0)*100)}%</p>
      {p.quantities&&<p>사입 시나리오: 보수 {p.quantities.conservative} / 기준 {p.quantities.recommended} / 공격 {p.quantities.aggressive}개</p>}
      {p.missing?.length>0&&<p>추가 확인: {p.missing.join(" · ")}</p>}
    </details>
  </article>;
}
...
export default function HomeMdBriefing(){
  ...
  return <div className="md-report operator-home">
    <header className="op-head"><div><small>OARS DAILY</small><h1>오늘 할 일</h1><p>숫자보다 먼저, 지금 확인할 것만 보여줍니다.</p></div><button onClick={load} disabled={loading}>{loading?"확인 중":"새로고침"}</button></header>
    {error&&<p role="alert">{error}</p>}
    <section className="quick-grid">
      <a href="/?tool=sms"><span>주문·고객응대</span><b>신규 주문 확인 →</b></a>
      <a href="/?tool=post"><span>오늘 발송</span><b>우체국 출고 준비 →</b></a>
      <a href="/dashboard"><span>최근 7일 주문매출</span><b>{won(seven.sales)} →</b></a>
    </section>
    <section className="op-section"><div className="section-title"><div><small>MD PRIORITY</small><h2>오늘 확인할 상품</h2></div><span>{priority.length}개</span></div>
      {priority.length?priority.map(p=><Card key={p.key} p={p}/>):<p className="empty">오늘 바로 판단할 상품은 없습니다.</p>}
      {rows.length>priority.length&&<details className="all-products"><summary>나머지 상품 {rows.length-priority.length}개 보기</summary>{rows.filter(p=>!priority.includes(p)).map(p=><Card key={p.key} p={p}/>)}</details>}
    </section>
    <section className="op-section compact"><div className="section-title"><div><small>STATUS</small><h2>MD 상태 요약</h2></div></div><div className="status-grid"><div><b>{buy.length}</b><span>사입 검토</span></div><div><b>{watch.length}</b><span>관찰</span></div><div><b>{stop.length}</b><span>추가사입 중단</span></div></div></section>
    {report.warnings?.length>0&&<details className="data-check"><summary>⚠ 데이터 확인 {report.warnings.length}건</summary><p>아래 내용은 분석용 점검 정보입니다. 실제 수정이 필요한 항목만 확인하면 됩니다.</p>{report.warnings.map((w,i)=><p key={i}>{w}</p>)}</details>}
    <style jsx>{`
