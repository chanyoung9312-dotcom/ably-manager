...
function isNew(r){return NEW_CODES.has(text(r.status));}
...
export default function SmsCafe24Manager(){
 ...
 const newRows=useMemo(()=>rows.filter(isNew),[rows]),preparedRows=useMemo(()=>rows.filter(isPrepare),[rows]),shown=tab==='new'?newRows:preparedRows;
 return <>
