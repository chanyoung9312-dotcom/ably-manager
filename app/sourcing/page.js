...
function ResearchPlanBoard({ researchPlan }) {
  ...
  const copy = async (text) => {
    try {
      ...
      // Copy is a convenience only; the visible keyword remains available.
    }
  };
            ...
            <section className="research-keywords">
              ...
              <div className="research-keyword-list">
                {task.searchKeywords.map((item) => (
                  <button
                    type="button"
                    className={`research-keyword ${item.evidence}`}
                    key={`${item.evidence}:${item.keyword}`}
                    title={`${item.evidenceLabel} · 클릭해서 복사`}
  ...
  return (
    <main className="sourcing-page">
      ...
      <style jsx global>{`
        ...
        @media(min-width:1120px){.brief-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.candidate-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.card-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.detail-grid{grid-template-columns:repeat(2,1fr)}.period-grid{grid-template-columns:1fr}}
